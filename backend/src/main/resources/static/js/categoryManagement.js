document.addEventListener('DOMContentLoaded', () => {
    ...
    if(closeIconModalBtn) closeIconModalBtn.addEventListener('click', closeIconModal);
    ...
    document.querySelectorAll('.remove-icon').forEach(removeBtn => {
        removeBtn.addEventListener('click', (e) => {
            const iconId = e.target.closest('.remove-icon').getAttribute('data-icon-id');
    ...
    document.querySelectorAll('.delete').forEach(removeBtn => {
        removeBtn.addEventListener('click', (e) => {
            const categoryId = e.target.closest('.delete').getAttribute('data-category-id');
            callApiDelete(categoryId, `${window.location.origin}/fin-flow/api/categories`)
        });
    });
    ...
    document.querySelectorAll('.icon-option').forEach(option => {
        option.addEventListener('click', (e) => {
            ...
            document.getElementById('category-icon-input').value = option.getAttribute('data-icon-id');
        });
    });
});
