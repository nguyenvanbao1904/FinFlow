...
export const logoutUser = (navigate) => async (dispatch) => {
    try{
        let response = await publicApis.post(endpoints.auth.logout, {token: cookie.load("token")});
...
export const outboundLogin = (authCode, navigate) => async (dispatch) => {
    try {
        let response = await publicApis.post(`${endpoints.auth.outbound}?code=${authCode}`, null);
        dispatch(login({ token: response.data.data.token }));
        cookie.save("token", response.data.data.token);
...
export const introspect = (token) => async (dispatch) => {
    try {
        ...
        console.error("Introspection error:", error.response.data.message);
        cookie.remove("token");
    }
}
